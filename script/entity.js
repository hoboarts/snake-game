...
class Snake extends Entity {
    constructor() {
        ...
        this.vector = [];//snakeHead vector RAW not converted in cell
        this.snakeSize = 1;
        this.positionsArr = [];
        this.direction = "SN"; // default "SN" - meaning TO-*S*OUTH-FROM-*N*ORTH
        this.histDirArr = ["SN"];
        this.fTailAp = true;
    }
        ...
        if (this.snakeSize < 2) {
            ...
            while (counter - 1 < this.snakeSize) {
                if (counter === 1) {
                    ...
                    if (this.fTailAp) {
                        Snake.setElemToPos("s2", this.positionsArr[0].split("x"));
                        this.fTailAp = false;// MAYBE MOD LATE
                    } else { Snake.setElemToPos("s2", this.positionsArr[1].split("x")); }
                    counter++;
                } else {
    ...
    bodyDetect() {//DETECT GAME OVER OR NOT
        let currPos = getVecToCell(this.vector),
            counter = 1;
        while (counter < this.positionsArr.length - 1) {
            let exArr = this.positionsArr[counter].split("x"),
                comX = Number(exArr[0]),
                comY = Number(exArr[1]);
